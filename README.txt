...
Model variables are saved in "saves_*MODEL*" directories.
...
All code is run from main.py. Uncomment the appropriate line to run code. Be cautious of trying to restore a model if save files are not present. Also ensure that all the tfrecord files are written with ld.create_dataset before
trying to run the learning algorithms